...
*Modified on:* November 30th, 2022
...
#shapiro.test(aim1_men$maze_path_efficiency) #tbd

shapiro.test(aim1_men$dsp_si_true_pct) #yes
shapiro.test(aim1_men$dsp_success_pct) #no
shapiro.test(aim1_men$dsp_path_efficiency_new) #no
...
Degrees traveled at 3.0m was negatively correlated with testosterone ($\rho$ = -0.484, *p* < 0.05). No other significant correlations for position error or angular error.
...
#shapiro.test(aim1_men_maze$maze_path_efficiency) #tbd
...
No significant correlations for accuracy or moves. Waiting on path efficiency data.
...
aim1_men_dsp <- aim1_men %>% 
  dplyr::select(subject_id, dsp_useable, 
                dsp_success_pct, dsp_si_true_pct, dsp_path_efficiency_new,
                testosterone_spatial_ng_dl) %>% 
    filter(dsp_useable == "Yes",
         is.na(testosterone_spatial_ng_dl) != TRUE)
#n=28
...
shapiro.test(aim1_men_dsp$dsp_path_efficiency_new) #no
...
########## pearson for path efficiency
cor.test(aim1_men$testosterone_spatial_ng_dl,
         aim1_men$dsp_path_efficiency_new,
         conf.level = 0.95,
         method = "pearson")
```
...
hormone_mod <- aov(progesterone_spatial_ng_ml ~ repo_status_spatial,
      data = aim1_women)
TukeyHSD(hormone_mod)
...
shapiro.test(aim1_women$dsp_path_efficiency_new) #no
...
aim1_women_loop <- aim1_women %>% 
  dplyr::select(subject_id, repo_status_spatial, loop_useable, 
                loop_pe_rad3_m, 
                loop_de_rad3_degree, loop_de_avg_degree,
                loop_ae_rad3_degree, loop_ae_avg_degree,
...
aim1_women_maze <- aim1_women %>% 
  dplyr::select(subject_id, repo_status_spatial, maze_useable, 
                maze_accuracy_pct, maze_moves, maze_path_efficiency,
                estradiol_spatial_pg_ml,
                progesterone_spatial_ng_ml,
                fsh_spatial_miu_ml) %>% 
...
#shapiro.test(aim1_women_maze$maze_path_efficiency) #tbd
...
No correlations for accuracy or moves made. Waiting for path efficiency data.
...
aim1_women_dsp <- aim1_women %>% 
  dplyr::select(subject_id, repo_status_spatial, dsp_useable, 
                dsp_success_pct, dsp_si_true_pct, dsp_path_efficiency_new,
                estradiol_spatial_pg_ml,
                progesterone_spatial_ng_ml,
                fsh_spatial_miu_ml) %>% 
...
shapiro.test(aim1_women_dsp$dsp_path_efficiency_new) #no
...
#Estradiol
cor.test(aim1_women_dsp$estradiol_spatial_pg_ml,
         aim1_women_dsp$dsp_path_efficiency_new,
         conf.level = 0.95,
         method = "spearman")
#Progesterone
cor.test(aim1_women_dsp$progesterone_spatial_ng_ml,
...
```{r dsp-anova}
si_mod <- aov(dsp_si_true_pct ~ repo_status_spatial,
              data = aim1_women_dsp)
summary(si_mod)
```
...
#save
ggsave("study1_dsp_e_fsh_spatial_gam.png")
```
...
- No significant correlations observed among men.
...
- While there were no significant correlations for DSP wayfinding success, there was a positive correlation between estradiol and solution index ($\rho$ = 0.421, *p* < 0.01, Spearman), and a marginal negative association between FSH and solution index ($\rho$ = -0.281, *p* = 0.058, Spearman). There was no significant correlation with progesterone and solution index ($\rho$ = 0.16, *p* = 0.29, Spearman). This was true when using hormone measurements from spatial timepoints. The statistics reported in this summary are using the hormones from when spatial study occurred.
