...
aim1_women %>% 
  ...
  summarize(count = n()) %>% 
  kable(col.names = c("Reproductive Status", "n"))
```
...
For measures, we will use the same dependent variables as in  our previous work:
...
No correlation for wayfinding success, but significant positive correlation for solution index for estradiol ($\rho$ = 0.421, *p* < 0.01) and negative marginal correlation for FSH ($\rho$ = -0.281, *p* = 0.058).
...
With the one-tailed test, there was also no relationship for wayfinding success, but solution index remained positively correlated with estradiol ($\rho$ = 0.421, *p* < 0.01) and was significantly negatively correlated with FSH

wayfinding success, but significant positive correlation for solution index for estradiol  and negative marginal correlation for FSH ($\rho$ = -0.281, *p* < 0.05).
...
##### Is there a sex difference in place strategy proportion used? #####
...
There were no differences between proportion of place strategies used between women and men, but if we break it down to look at it by reproductive status, the pres actually have higher place strategy use than men.
...
aim1_women_loop <- aim1_women %>% 
  dplyr::select(subject_id, repo_status, loop_useable, 
                ...
                fsh_spatial_miu_ml) %>% 
  filter(loop_useable == "Yes")
#n=32, but 2 without 3.0m so n=31
...
This was calculated to remove any variability from estradiol
...
```{r raio-correlations-one-tail}
#One-tailed t-test for E and FSH for solution index
cor_e <- cor.test(aim1_women_dsp$estradiol_spatial_pg_ml,
         aim1_women_dsp$dsp_si_true_pct,
...
DescTools::SpearmanRho(aim1_women$estradiol_spatial_pg_ml,
                       aim1_women$dsp_si_true_pct,
                       use = "complete.obs"
                       conf.level	= 0.95)

DescTools::SpearmanRho(aim1_women$fsh_spatial_miu_ml,
                       aim1_women$dsp_si_true_pct,
                       use = "complete.obs",
                       conf.level	= 0.95)
```
...
The FSH:E ratio and DSP solution index plot has already been removed for one outlier (post-menopause).
...
For Table 1, we want to check if there are any sex differences between men and women for the different navigation variables.
...
For Table, calculating t-test or Wilcoxon rank sum test to test sex differences for spatial navigation measures reveal that there is a sex difference for DSP wayfinding success (men were more successful at reaching target) and LOOP degrees traveled (men travel fewer degrees on circular path).
