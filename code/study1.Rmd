...
tab_final %>% 
  ...
  kable_styling("striped", full_width = FALSE,
                position = "center", 
                font_size = 12) %>%
  add_header_above(c("Overall" = 4, "LOOP" = 3, "MAZE" = 3, "DSP" = 3))
```
...
Hormones are testosterone for men; and estradiol, progesterone, FSH for women. Hormones are taken during time of spatial navigation task.
...
Spearman rank correlations were used for MAZE accuracy and DSP solution index. All other correlations for men were Pearson.
...
aim1_men_loop <- aim1_men %>% 
  dplyr::select(subject_id, loop_useable, loop_pe_avg_m, loop_de_avg_degree,
...
#Normality test
shapiro.test(aim1_men_loop$loop_pe_avg_m) #no
...
aim1_men_maze <- aim1_men %>% 
  dplyr::select(subject_id, maze_useable, maze_accuracy_pct, maze_moves,
                testosterone_spatial_ng_dl) %>% 
  filter(maze_useable == "Yes",
         is.na(testosterone_spatial_ng_dl) != TRUE)
#n=23
...
shapiro.test(sqrt(aim1_men_maze$maze_accuracy_pct)) #yes
...
aim1_men_dsp <- aim1_men %>% 
  dplyr::select(subject_id, dsp_useable, dsp_success_pct, dsp_si_true_pct,
                testosterone_spatial_ng_dl) %>% 
    filter(dsp_useable == "Yes",
         is.na(testosterone_spatial_ng_dl) != TRUE)
#n=28
...
shapiro.test(sqrt(aim1_men_dsp$dsp_si_true_pct)) #no
...
cor.test(aim1_men$testosterone_spatial_ng_dl,
         ...
         method = "spearman")
cor.test(aim1_men$testosterone_spatial_ng_dl,
         sqrt(aim1_men$dsp_si_true_pct),
         conf.level = 0.95,
         method = "pearson")
```

No correlations for wayfinding success or solution index
...
#Normality test
shapiro.test(aim1_women_loop$loop_pe_avg_m) #no
...
No correlation for wayfinding success, but significant positive correlation for solution index for estradiol ($\rho$ = 0.42, *p* < 0.01) and negative marginal correlation for FSH ($\rho$ = -0.28, *p* = 0.058).
...
- While there were no significant correlations for DSP wayfinding success, there was a positive correlation between estradiol and solution index ($\rho$ = 0.42, *p* < 0.01, Spearman), and a marginal negative association between FSH and solution index ($\rho$ = -0.28, *p* = 0.058, Spearman). There was no significant correlation with progesterone and solution index ($\rho$ = 0.16, *p* = 0.29, Spearman). This was true when using hormone measurements from spatial timepoints. The statistics reported in this summary are using the hormones from when spatial study occurred.

- When using estradiol to FSH ratio (or FSH to estradiol ratio) as a single measure for hormone concentration, the relationship with DSP solution index remained significant (*p* < 0.01, Spearman).
