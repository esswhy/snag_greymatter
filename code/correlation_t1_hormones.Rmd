...
Shapiro-Wilk's test reveal that LOOP dependent variables and DSP wayfinding success are normally distributed, but MAZE dependent variables and DSP solution index are not even after log and square root transformations. Thus, all correlations with GMV for these DV will be spearman rank correlations.
...
MAZE dependent variables and DSP solution index success were significantly different than normal. MAZE and DSP solution index was still skewed after transformations, so correlations with MAZE variables and DSP solution index for women will be using Spearman rank correlation.
...
LOOP variables, DSP wayfinding success, and MAZE moves variables were not significantly different than normal, but MAZE accuracy and DSP solution index were. Despite transformations, MAZE moves and DSP solution index accuracy was still pretty skewed so Spearman rank correlations will be applied for associations with GMV for men here.
...
#By sex
c1a <- ggscatter(data_loop_women, x = "avg_t1_hipp", y = "loop_pe_avg_m", color = "red", alpha = 0.6, title = "Women",
          add = "reg.line",
          add.params = list(color = "black", fill = "lightgray"), # Customize reg. line
          cor.coef = TRUE, # Add correlation coefficient. see ?stat_cor
          conf.int = TRUE, 
     ...
     theme_bw() +
     theme(legend.position = "none")
c1b <- ggscatter(data_loop_men, x = "avg_t1_hipp", y = "loop_pe_avg_m", color = "#00AFBB", alpha = 0.6, title = "Men",
          add = "reg.line",
          add.params = list(color = "black", fill = "lightgray"), # Customize reg. line
          cor.coef = TRUE, # Add correlation coefficient. see ?stat_cor
          conf.int = TRUE, 
...
#By repo
c1c <- ggscatter(data_loop_women, x = "avg_t1_hipp", y = "loop_pe_avg_m", color = "repo_status",
          add = "reg.line",
          add.params = list(color = "black", fill = "lightgray"), # Customize reg. line
          cor.coef = TRUE, # Add correlation coefficient. see ?stat_cor
          conf.int = TRUE, 
...
All are Pearson correlations. Larger T1 hipp volumes is positively associated with position error in pres (*p* < 0.05, Pearson).
...
- For LOOP, larger T1 hipp GMV is positively correlated with position error for pres (*p* < 0.05, Pearson). Women with higher GMV correlated with fewer degrees traveled (*p* < 0.05, Pearson). Pres who had higher GMV also correlated with fewer degrees traveled (*p* < 0.01, Pearson). 
