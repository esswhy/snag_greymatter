...
- Sum up the averaged CA1, CA23, DG, SUB to get T2 total hippocampal volume
...
Still waiting on 4 subjects to process
...
- MAZE: *n* = 14 DHEAS, *n* = 15 for SHBG and T

- DSP: *n* = 19 DHEAS, *n* = 20 for SHBG and T


We are still waiting for 4 more subject's (2 female, 2 male) segmentation to be compelted.
...
**Subfields:** Shapiro-Wilk's test reveal that CA1 for overall and women, ERC for overall, and PHC for women volumes were not normally distributed. Hormone and subfield/volume associations for these subfields will be using Spearman rank correlations.
...
```{r include=TRUE, echo=FALSE}
#All
c1 <- ggscatter(data_loop, x = "avg_total_hipp", y = "loop_pe_avg_m", color = "repo_status",
          add = "reg.line",
          add.params = list(color = "black", fill = "lightgray"), # Customize reg. line
          cor.coef = TRUE, # Add correlation coefficient. see ?stat_cor
          conf.int = TRUE, 
...
#By sex
c1a <- ggscatter(data_loop_women, x = "avg_total_hipp", y = "loop_pe_avg_m", color = "red", alpha = 0.6, title = "Women",
          add = "reg.line", 
          add.params = list(color = "black", fill = "lightgray"), # Customize reg. line
          cor.coef = TRUE, # Add correlation coefficient. see ?stat_cor
          conf.int = TRUE, 
     ...
     theme_bw() +
     theme(legend.position = "none")
c1b <- ggscatter(data_loop_men, x = "avg_total_hipp", y = "loop_pe_avg_m", color = "#00AFBB", alpha = 0.6, title = "Men",
          add = "reg.line",
          add.params = list(color = "black", fill = "lightgray"), # Customize reg. line
          cor.coef = TRUE, # Add correlation coefficient. see ?stat_cor
          conf.int = TRUE, 
...
#By repo
c1c <- ggscatter(data_loop_women, x = "avg_total_hipp", y = "loop_pe_avg_m", color = "repo_status", 
          add = "reg.line",
          add.params = list(color = "black", fill = "lightgray"), # Customize reg. line
          cor.coef = TRUE, # Add correlation coefficient. see ?stat_cor
          conf.int = TRUE, 
...
```{r include=TRUE, echo=FALSE}
#All
c1 <- ggscatter(data_loop, x = "avg_mtl", y = "loop_pe_avg_m", color = "repo_status",
          add = "reg.line",
          add.params = list(color = "black", fill = "lightgray"), # Customize reg. line
          cor.coef = TRUE, # Add correlation coefficient. see ?stat_cor
          conf.int = TRUE, 
...
#By sex
c1a <- ggscatter(data_loop_women, x = "avg_mtl", y = "loop_pe_avg_m", color = "red", alpha = 0.6, title = "Women",
          add = "reg.line", 
          add.params = list(color = "black", fill = "lightgray"), # Customize reg. line
          cor.coef = TRUE, # Add correlation coefficient. see ?stat_cor
          conf.int = TRUE, 
     ...
     theme_bw() +
     theme(legend.position = "none")
c1b <- ggscatter(data_loop_men, x = "avg_mtl", y = "loop_pe_avg_m", color = "#00AFBB", alpha = 0.6, title = "Men",
          add = "reg.line",
          add.params = list(color = "black", fill = "lightgray"), # Customize reg. line
          cor.coef = TRUE, # Add correlation coefficient. see ?stat_cor
          conf.int = TRUE, 
...
#By repo
c1c <- ggscatter(data_loop_women, x = "avg_mtl", y = "loop_pe_avg_m", color = "repo_status", 
          add = "reg.line",
          add.params = list(color = "black", fill = "lightgray"), # Customize reg. line
          cor.coef = TRUE, # Add correlation coefficient. see ?stat_cor
          conf.int = TRUE, 
...
All correlations were Pearson except for overall (Spearman)
...
All correlations are Pearson except for overall (Spearman)
...
There are 4 subjects (2 female, 2 male) still waiting to be segmented, and they will be included in the next iteration of this analysis.
...
- For progesterone, higher concentration was negatively associated with lower total hipp volumes in women (*p* = 0.04, Spearman) and in pres (*p* < 0.001, Spearman). Progesterone was marginally negatively associated with lower MTL volumes in women (*p* = 0.05, Spearman) and in pres (*p* = 0.08, Spearman)


**Subfields**

- Subregion volumes for women and men were generally normally distributed, except for CA1 overall and in women, ERC overall, and PHC in women. ANOVA results comparing subfields by reproductive stage were all not significant. T-test results comparing subfields by sex were all not significant except for ERC (*p* < 0.05), with women showing greater ERC subfield volume than men.
...
- For progesterone, it was significantly negatively associated with SUB for pres (*p* < 0.05, Spearman), with PRC for posts (*p* < 0.05, Spearman), DG for women (*p* < 0.01, Spearman) and DG for pres (*p* < 0.01, Spearman), as well as with CA1 for pres (*p* = 0.15, Spearman). It was marginally negatively associated with PRC for women (*p* = 0.086, Spearman). Progesterone was only marginally positively associated with CA1 for posts (*p* = 0.052, Spearman). 

- For FSH, it was significantly negatively correlated with CA1 for posts (*p* < 0.05, Spearman). It was significantly positively correlated with ERC for peris (*p* < 0.05, Spearman) and SUB for pres (*p* < 0.05, Spearman). FSH was marginally positively correlated with CA23 for peris (*p* = 0.07, Spearman) and with DG for peris (*p* = 0.054, Spearman).
...
- For DHEAS, it was significantly negatively correlated with CA23 for posts (*p* < 0.01, Spearman) and with DG for posts (*p* < 0.05, Spearman). It was marginally negatively correlated with SUB for posts (*p* = 0.081, Spearman).

- For testosterone, it was marginally negatively correlated with CA1 for men (*p* = 0.075, Pearson) and with SUB for pres (*p* = 0.078, Spearman). Testosterone was also marginally positively associated with PHC for peris (*p* = 0.068, Spearman).
...
- DSP higher solution index was significantly negatively correlated with lower total hipp volume in men (*p* < 0.05, Pearson) and marginally negatively correlated with lower total hipp volume in posts (*p* = 0.08, Spearman). DSP higher solution index was significantly positively correlated with total hipp volume in peris (*p* < 0.05, Spearman).
...
- LOOP larger position error was significantly negatively correlated with lower PHC volume within overall (women and men) (*p* = 0.049, Pearson).

- LOOP greater degrees traveled was significantly negatively correlated with lower volumes in CA1 for overall (*p* < 0.05, Spearman) and for men (*p* = 0.05, Pearson), in CA23 for overall (*p* < 0.01, Pearson) and for men (*p* < 0.05, Pearson), in DG for overall (*p* < 0.05, Pearson) and for men (*p* < 0.05, Pearson), in ERC for peris (*p* = 0.046, Pearson), in PRC for peris (*p* = 0.46, Pearson), and in SUB for overall (*p* < 0.05, Pearson). Basically pattern can be seen in all subfields except for PHC.

- There were no significant or marginal correlations observed for MAZE accuracy.

- MAZE higher moves made was significantly positively associated with higher PHC volume in pres (*p* < 0.05, Spearman) but only marginally positively associated with higher CA1 volume in women (*p* = 0.088, Spearman).

- DSP higher wayfinding success was significantly negatively correlated lower ERC volumes in pres (*p* < 0.05, Pearson), but marginally with overall (*p* = 0.057, Spearman). It was marginally positively associated with PHC volumes in women (*p* = 0.09, Spearman) and in peris (*p* = 0.08, Spearman).

- DSP higher solution index was significantly positively correlated with higher volume in CA23 in peris (*p* < 0.05, Spearman). It was significantly negatively correlated with lower volume in DG for men (*p* < 0.05, Pearson) but marginally for posts (*p* = 0.074, Spearman).
